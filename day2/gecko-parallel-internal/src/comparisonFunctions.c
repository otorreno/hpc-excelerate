...
int readHashEntry(hashentry *h, FILE *f, int freqThr) {
	do {
		fread(h, sizeof(hashentry), 1, f);
		if (h->num > (unsigned int)freqThr) {
			fseek(f, h->num * sizeof(location), SEEK_CUR);
		}
	} while (!feof(f) && h->num > (unsigned int)freqThr);
...
void loadWordOcurrences(hashentry he, location** pos, FILE** f) {
	...
	fread(*pos, sizeof(location), he.num, *f);
}
